SGAR-MOT is released under the PolyForm Noncommercial License 1.0.0 
(SPDX identifier: PolyForm-Noncommercial-1.0.0), reproduced in full below.

Required Notice: Copyright (c) 2026 Universidade de Santiago de Compostela

WHAT THESE TERMS COVER
----------------------

These terms cover the code written for SGAR-MOT, that is, everything in
this repository except the third-party files listed in the next section:

  * src/sgar_mot/       -- MIR and ORP (except the file noted below)
  * src/bytetrack/      -- the tracker, evaluator, entry points and exps
  * src/configs/, docker/, docs/, requirements.txt, pyproject.toml

The released MIR checkpoint (pretrained/mir_sgar_mot.pth) is part of "the 
software" as these terms use the word, and is covered by them on the same 
footing as the code, so that one definition of "noncommercial purpose" 
governs the whole release. To the extent that the trained weights are 
protected by copyright or related rights, they are licensed under these 
terms; where they are not so protected, the authors ask that you respect 
the same limits. The weights were trained on MOTSynth, whose own terms 
are noncommercial.
...
Third-party code is redistributed here under its own licence, and nothing 
in this file restricts it:

  * src/sam2_sgar_mot/

        SAM 2, Copyright (c) Meta Platforms, Inc. and affiliates, with our 
        modifications marked [SGAR-MOT]. Apache License 2.0; the licence 
        text is in src/sam2_sgar_mot/LICENSE.

  * src/sgar_mot/mir/positional_encodings/torch_encodings.py

        Copyright (c) 2020 Peter Tatkowski. MIT licence; see 
        licenses/multidim-positional-encoding-MIT.txt.

  * portions of src/bytetrack/yolox/tracker/sgar_tracker.py,
    src/bytetrack/yolox/evaluators/sgar_evaluator.py,
    src/bytetrack/tools/track_sgar.py and src/bytetrack/exps/example/mot/*.py

        Derived from ByteTrack, Copyright (c) 2021 Yifu Zhang (MIT, see 
        licenses/ByteTrack-MIT.txt), and, in the evaluator, from YOLOX 
        Copyright (c) Megvii, Inc. and its affiliates (Apache License 2.0, 
        see licenses/Apache-2.0.txt). Each file states in its header which 
        parts come from where. Those parts remain available to you under 
        their original licences.

THIRD_PARTY.md records every component in detail.
